...
def _topk(scores, K=40):
    ...
    """
    # scores=heat batch=1 cat=9 height=128 width=168
    batch, cat, height, width = scores.size()

    # (b, c, K)
    # topk_scores=[1,9,100] topk_inds=[1,9,100]  传入K=100
...
def grasp_pose_decode(
    ...
    # num_kpts = (1,72,128,168) // 18 = 4
    num_kpts = kps.shape[1] // (2 * cls_num) 
    # centers=[1,100,2]   kps=[1,100,8]  w=[1,100,1]  scores=[1,100,1] clses=[1,100,1] scls=None scales=None
    # 中心位置=像素位置+偏差 关键点相对于中心点偏移  宽度  分数(hm的值)
