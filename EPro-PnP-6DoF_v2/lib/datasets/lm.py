...
class LM(data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            box = self.load_box(idx)        # box.txt 每个是四个数，表示bounding box
            pose = self.load_pose(idx)      # pose.txt 十二个数，R+t
            ...
            if (self.split == 'train') and self.cfg.dataiter.dzi:  # pad_tatio:1.5
                c, s = self.xywh_to_cs_dzi(box, self.cfg.augment.pad_ratio, s_max=max(ref.im_w, ref.im_h))  # 数据增强 
            else:
            ...
            rgb, c_h_, c_w_, s_ = zoom_in(rgb, c, s, self.cfg.dataiter.inp_res)     # 缩放obj
            rgb = rgb.transpose(2, 0, 1).astype(np.float32) / 255.                  # HWC转CHW，然后归一化
